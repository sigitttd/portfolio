...
const contactLinks = [
  ...
  {
    label: 'Phone',
    getHref: (info: typeof personalInfo) => `https://wa.me/${info.phone}`,
    isDownload: false,
    icon: (
      <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
  ...
  {
    label: 'Download CV',
    getHref: (_info: typeof personalInfo) => '/cvporto/Curriculum_Vitae_Rahmat_Sigit_Hidayat.pdf',
    isDownload: true,
    icon: (
      <svg className="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
            ...
            <motion.div style={{ y: textY }}>
              ...
              <div className="flex flex-wrap gap-2">
                {contactLinks.map((link, i) => {
                  const isCV = link.label === 'Download CV'
                  return (
                    <motion.a
                      key={link.label}
                      ...
                      className={
                        isCV
                          ? 'inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 bg-electric-blue/15 backdrop-blur-sm border border-electric-blue/50 text-electric-blue hover:bg-electric-blue/25 hover:border-electric-blue hover:shadow-glow-sm'
                          : 'inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-surface/80 backdrop-blur-sm border border-border-subtle hover:border-electric-blue/50 hover:bg-electric-blue/5 hover:text-electric-blue hover:shadow-glow-sm text-text-muted text-sm font-medium transition-all duration-200'
                      }
                    >
                      {link.icon}
                      {link.label}
